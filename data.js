const SchoolData = (() => {
  ...
  function isResponseOk(response) {
    ...
    return IS_FILE_PROTOCOL && response.status === 0;
  }
  ...
  async function loadSchools() {
    ...
    try {
      const fetched = await fetchText(dataPath);
      if (!fetched) {
  ...
  function normalizePrefectureSlug(value) {
    return String(value || "")
      .toLowerCase()
      .replace(/[^a-z0-9-]/g, "");
  }
  ...
  function getPrefectureSlug() {
    ...
    const raw = params.get("pref") || params.get("prefecture") || params.get("region");
    const normalized = normalizePrefectureSlug(raw);
    if (normalized) {
      try {
        localStorage.setItem("prefectureSlug", normalized);
      } catch (error) {
    ...
    let stored = "";
    try {
      stored = localStorage.getItem("prefectureSlug") || "";
    } catch (error) {
      stored = "";
    }
    const storedNormalized = normalizePrefectureSlug(stored);
    return storedNormalized || DEFAULT_PREFECTURE;
  }
